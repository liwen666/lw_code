...
BEGIN
  ...
  FOR C_COL IN (SELECT X.AGENCYID,X.PROJTYPEID,X.EXPECOID,Z.DBCOLUMNNAME,T.DBTABLENAME
    ...
    WHERE X.FORMULAID = Y.FORMULAID AND Y.TABLEID = Z.TABLEID AND X.FUNDSOURCEID = Z.COLUMNID AND T.TABLEID = Y.TABLEID  AND Z.ISLEAF = '1' AND (Y.STANDARDID LIKE '%'||V_STNDID||',%' OR Y.STANDARDID LIKE '%'||V_NEWID||',%')) LOOP
    ...
    END IF;
  END LOOP;
END STND_P#CAL_T_AGSTANDGRADE_TR;--STND_P#CAL_T_AGSTANDGRADE_TR
