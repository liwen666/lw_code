create or replace procedure SPF_AND_BGT_INTERFACE(p_districtid in varchar2,p_docid in varchar2,p_batchid in varchar2) as
--项目库和预算接口存储过程
...
v_sql_01:=substr(v_sql_01,1,length(v_sql_01) - 1)||' ';
v_sql_02:=substr(v_sql_02,1,length(v_sql_02) - 1)||' ';
v_sql_03:=substr(v_sql_03,1,length(v_sql_03) - 1)||' ';
v_sql_04:=substr(v_sql_04,1,length(v_sql_04) - 1)||' ';
v_sql_05:=substr(v_sql_05,1,length(v_sql_05) - 1)||' ';
v_sql_06:=substr(v_sql_06,1,length(v_sql_06) - 1)||' ';
v_sql_07:=substr(v_sql_07,1,length(v_sql_07) - 1)||' ';
v_sql:='
          ...
          'from
               (select i.PROJECTID,
                 ...
                 where i.ISPLANVALID = ''1''
                   ...
                   and b1.SPFID in ( select taskid from spf_t_oarelation where docid='''||p_docid||''' and tasktype=''0'')
                  ) t
         where
            t.ISBGT = ''1''
           and Exists (select 1
          ...
          'from (
                select s1.PROJECTID,
                 ...
                 where i.PROJECTID = s1.PROJECTID
                   ...
                   AND i.ISPLANVALID = ''1''
                   and s1.ISBGT = ''1''
                   and f1.SPFID in ( select taskid from spf_t_oarelation where docid='''||p_docid||''' and tasktype=''0'')
                   and Exists
                 (select 1
                          from code_t_agency_spf
