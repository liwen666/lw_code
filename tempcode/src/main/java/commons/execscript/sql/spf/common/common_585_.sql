...
BEGIN
  ...
  IF P_ISRECREATE = '0' THEN
    V_TABLEID := '36DA6D0F1C750084E0530A700187AB0D';
    V_NAME := P_CREATED_OBJ_NAME;
    V_CNNAME := P_CREATED_OBJ_CN_NAME;
		SELECT MAX(SUBSTR(DBTABLENAME,-3)) INTO V_SUFIX FROM P#DICT_T_MODEL WHERE REGEXP_LIKE(DBTABLENAME,'^'||V_NAME||'[0-9]{3}$');
		V_SUFIX := NVL(TRIM(V_SUFIX),'000');
		V_NAME := V_NAME||TRIM(TO_CHAR(TO_NUMBER(V_SUFIX) + 1,'000'));
  ...
  ELSE
    SELECT TABLEID,NAME,DBTABLENAME INTO V_TABLEID,V_CNNAME,V_NAME FROM DICT_T_MODEL WHERE APPID = P_APPID AND DEALTYPE = P_DEALTYPE;
    
		IF NOT REGEXP_LIKE(V_NAME,'^'||P_CREATED_OBJ_NAME||'[0-9]{3}$') THEN
			SELECT MAX(SUBSTR(DBTABLENAME,-3)) INTO V_SUFIX FROM P#DICT_T_MODEL WHERE REGEXP_LIKE(DBTABLENAME,'^'||P_CREATED_OBJ_NAME||'[0-9]{3}$')
			...
			V_SUFIX := NVL(TRIM(V_SUFIX),'000');
			V_NAME := V_NAME||TRIM(TO_CHAR(TO_NUMBER(V_SUFIX) + 1,'000'));
			UPDATE DICT_T_MODEL SET DBTABLENAME = V_NAME WHERE TABLEID = V_TABLEID;
		ELSE
			V_NAME := V_NAME;
		END IF;
    ...
    --删除
    DELETE FROM DICT_T_FACTOR WHERE TABLEID = V_TABLEID AND (ISLEAF = '0' OR ISLEAF = '1' AND DBCOLUMNNAME NOT IN ('PROJNAME','PROJCODE','PROJTYPEID','SPFID','CKMX','XGGW','PROJ_GROUP_NAME')) ;
  ...
  WHERE TABLEID = V_TABLEID AND ISLEAF = '1' AND DBCOLUMNNAME NOT IN ('AGENCYID','PROJNAME','PROJCODE','PROJTYPEID','SPFID','CKMX','XGGW','PROJ_GROUP_NAME','DISTRICTID');
  
  V_SQL := 'CREATE OR REPLACE VIEW '||V_NAME||' AS
SELECT '||V_TMP||'Y.PROJNAME,Y.PROJCODE,Y.PROJTYPEID ,Y.SPFID,CAST('''' AS VARCHAR2(255)) CKMX,CAST('''' AS VARCHAR2(255)) XGGW,Y.PROJNAME||''{''||Y.PROJECTID||''}''  PROJ_GROUP_NAME,Y.AGENCYID,
(SELECT DISTRICTID FROM SPF_T_FBASEINFO F WHERE F.SPFID = Y.SPFID) DISTRICTID
FROM '||V_SRC_TABLE||' X,SPF_T_PBASEINFO Y,SPF_T_PBASESTATUS PS
WHERE X.PROJECTID = Y.PROJECTID AND Y.PROJECTID = PS.PROJECTID';
/*
UNION ALL
