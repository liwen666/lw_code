...
FOR C_TAB IN (
       select  t.PROJECTID PROJECTID,
               ...
               (select projtype from spf_t_fbaseinfo a,spf_t_projecttype b  where a.spfid=t.spfid and a.projtypeid=b.projtypeid and rownum=1) PROJTYPE,
               t.EXPFUNCID ,
               t.EXPECOID'||v_sql_02||
          ' from
               (select i.PROJECTID ,
...
FOR C_TAB IN (
   ...
   ' from (
                select i.EXPFUNCID,
                 ...
                 '  from Spf_t_Pinvestplan i ,spf_t_pbaseinfo s1, spf_t_fbaseinfo f1
    ...
    then
      FOR C_TAB IN (
       ...
       ' from ( select i.GOVEXPFUNCID EXPFUNCID,
                  ...
                  from spf_t_pbaseinfo s1,Spf_t_Pinvestplan i,code_t_agency_spf ag
                 where s1.ISBGT = ''1''
                   and s1.projectid in '||p_projectids||'
                   and i.ISPLANVALID = ''1''
                   and (i.ISADJUST=''0'' or i.ISADJUST is null)
...
END LOOP;
   FOR C_TAB2 IN (
     select i.DATAKEY as INVEST_DATAKEY,m.DATAKEY AS INBUDGET_DATAKEY from spf_t_pinvestplan i,spf_t_pbaseinfo a1,exp_t_pbudgettemp m  where
     i.ISPLANVALID =''1''
     and i.PROJECTID = a1.PROJECTID
     and (i.ISADJUST=''0'' or i.ISADJUST is null)
     and a1.isbgt=''1''
