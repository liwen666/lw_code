CREATE OR REPLACE PROCEDURE P_KEEP_BGT_ACCOUNTS(P_WHERE VARCHAR2) IS
--预算记账
  ...
  BEGIN
    ...
    WHERE T.PROVINCE = V_PROVINCE AND T.YEAR = V_YEAR  AND T.OPERATION_TYPE = 'P_KEEP_BGT_ACCOUNTS'),
  ...
  FROM P#'||V_TABLE||' 
  WHERE YEAR = GLOBAL_MULTYEAR_CZ.SECU_F_GLOBAL_PARM(''YEAR'') AND SPFID IN (SELECT SPFID FROM SPF_T_FBASEINFO 
...
,NVL((SELECT DEPTID FROM SPF_V_FBASEINFO_A WHERE SPFID = Y.SPFID),
    ...
    SELECT T1.* FROM }'||V_TABLE|| ' T1
    WHERE T1.STATUS = ''1'' AND T1.DBVERSION >= :1 AND PROJECTID <> ''*'' AND '||P_WHERE||' AND NOT EXISTS(SELECT 1 FROM BGT_T_YSLYWHB WHERE SUPERID = T1.DATAKEY)
    ) TT
  ...
  WHERE TA.SFJZ = '0' AND TA.ISWRITEOFF = '0' AND EXISTS(SELECT 1 FROM }'||Q'{(SELECT Y.FINYEAR,Y.AGENCYID AGENCYGUID,Y.PROJECTID PROGUID,'01687gDiFwDjc' AGENCYEXPGUID, '' EXPFUNCGUID,'01687E2hS1Edv' FUNDTYPEGUID,Y.EXPECOGUID,Y.FUNDSOURCEGUID,Y.AMT,Y.DATAKEY,Y.DBVERSION
   ...
   SELECT T1.* FROM }'||V_TABLE|| ' T1
   WHERE T1.STATUS = ''1'' AND T1.DBVERSION >= :2 AND (EXPECOID IS NOT NULL AND EXPFUNCID IS NOT NULL) AND PROJECTID <> ''*'' AND '||P_WHERE||' AND NOT EXISTS(SELECT 1 FROM BGT_T_YSLYWHB WHERE SUPERID = T1.DATAKEY)
   ) TT
  ...
  AND EXISTS (SELECT 1 FROM (SELECT Y.FINYEAR,Y.AGENCYID AGENCYGUID,Y.PROJECTID PROGUID,'01687gDiFwDjc' AGENCYEXPGUID, EXPFUNCID EXPFUNCGUID,'01687E2hS1Edv' FUNDTYPEGUID,Y.EXPECOGUID,Y.FUNDSOURCEGUID,Y.AMT,Y.DATAKEY,Y.DBVERSION
     ...
     SELECT FINYEAR,DATAKEY,AGENCYID,PROJECTID, EXPECOID EXPECOGUID, FUNDSOURCEGUID, AMT,DBVERSION,EXPFUNCID  FROM (
     SELECT T1.* FROM }'||V_TABLE|| ' T1
     WHERE T1.STATUS = ''1'' AND T1.DBVERSION >= :1 AND (EXPECOID IS NOT NULL AND EXPFUNCID IS NOT NULL) AND PROJECTID <> ''*'' AND '||P_WHERE||' AND NOT EXISTS(SELECT 1 FROM BGT_T_YSLYWHB WHERE SUPERID = T1.DATAKEY)
     ) TT
     UNPIVOT(AMT FOR FUNDSOURCEGUID IN('||V_TMP||'))) Y) TB
     WHERE (TA.DATAKEY = TB.DATAKEY AND TA.FUNDSOURCEGUID = TB.FUNDSOURCEGUID 
     AND TA.FINYEAR||''$''||TA.AGENCYGUID||''$''||TA.PROGUID||''$''||TA.EXPFUNCGUID||''$''||TA.EXPECOGUID||''#''||TO_CHAR(TA.AMT)
  ...
  SELECT TA.* FROM }'||V_TABLE|| ' TA
  WHERE TA.STATUS = ''1'' AND TA.DATAKEY = C_DATA.DATAKEY AND TA.DBVERSION >= :2 AND (EXPECOID IS NOT NULL AND EXPFUNCID IS NOT NULL) AND PROJECTID <> ''*'' AND '||P_WHERE||' ) TT
...
SELECT FINYEAR,DATAKEY,AGENCYID,PROJECTID, EXPECOID EXPECOGUID, FUNDSOURCEGUID, AMT,DBVERSION,EXPFUNCID EXPFUNCGUID,SPFID  FROM (
SELECT TA.* FROM }'||V_TABLE|| ' TA
WHERE TA.STATUS = ''1'' AND TA.DBVERSION >= :1 AND (EXPECOID IS NOT NULL AND EXPFUNCID IS NOT NULL) AND PROJECTID <> ''*'' AND '||P_WHERE||' ) TT
 ...
 WHERE DATAKEY = Y.DATAKEY AND FUNDSOURCEGUID = Y.FUNDSOURCEGUID
    ...
    AND T.OPERATION_TYPE = 'P_KEEP_BGT_ACCOUNTS';
    
    INSERT INTO BGT_T_KEEPCOUNT_TIME(PROVINCE,YEAR,OPERATION_TYPE,LAST_OPERATION_TIME)
    VALUES(V_PROVINCE,V_YEAR,'P_KEEP_BGT_ACCOUNTS',V_DBVERSION);
    RETURN;
  END P_KEEP_BGT_ACCOUNTS;
--P_KEEP_BGT_ACCOUNTS
