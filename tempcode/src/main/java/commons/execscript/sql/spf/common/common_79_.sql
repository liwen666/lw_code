CREATE OR REPLACE PROCEDURE P_KEEP_ACCOUNTS9(P_WHERE VARCHAR2) IS
  ...
  BEGIN
    V_DBVERSION := SYSDATE;
    V_WHERE := Q'{AGENCYID IN (SELECT GUID FROM CODE_T_DISTRICT_SPF)}';
    ...
    WHERE T.PROVINCE = V_PROVINCE AND T.YEAR = V_YEAR  AND T.OPERATION_TYPE = 'P_KEEP_ACCOUNTS9'),
    ...
    WHERE TABLE_NAME = 'BGT_T_YSJZSJB_ADJUST' AND COLUMN_NAME NOT IN ('GUID','AMT','SFJZ','ISWRITEOFF');
  ...
  FROM P#'||V_TABLE||'
  WHERE YEAR = GLOBAL_MULTYEAR_CZ.SECU_F_GLOBAL_PARM(''YEAR'') AND ISADJUST = ''1'' AND SPFID IN (SELECT SPFID FROM SPF_T_FBASEINFO
  WHERE DISTRICTID = (SELECT GUID FROM CODE_T_DISTRICT_SPF WHERE CODE = GLOBAL_MULTYEAR_CZ.SECU_F_GLOBAL_PARM(''DIVID'')))
    ...
    V_SQL := Q'{DELETE FROM BGT_T_YSJZSJB_ADJUST T1 WHERE SFJZ IN ('0', '2') AND ISWRITEOFF ='0' AND EXISTS (SELECT 1 FROM }'||V_TABLE||Q'{ T2 WHERE T2.STATUS = '2' AND T2.DBVERSION >= :1 AND T2.DATAKEY = T1.DATAKEY)}';
    BEGIN
    ...
    V_SQL := 'INSERT INTO BGT_T_YSJZSJB_ADJUST('||V_COLLIST||',AMT,SFJZ,ISWRITEOFF)
    SELECT '||V_COLLIST||Q'{,0-AMT AMT,'0' SFJZ,'1' ISWRITEOFF
    FROM BGT_T_YSJZSJB_ADJUST T1 WHERE SFJZ = '1' AND ISWRITEOFF ='0'
    AND EXISTS (SELECT DATAKEY FROM }'||V_TABLE||Q'{ T2 WHERE T2.STATUS = '2' AND T2.DBVERSION >= :1 AND T2.DATAKEY = T1.DATAKEY)}';
    BEGIN
      --DBMS_OUTPUT.PUT_LINE(V_SQL);
      EXECUTE IMMEDIATE V_SQL USING V_LASTMODIFIED;
    EXCEPTION WHEN OTHERS THEN
    ...
    V_SQL := Q'{UPDATE BGT_T_YSJZSJB_ADJUST T1
    SET ISWRITEOFF = '1'
    ...
    V_SQL := 'UPDATE BGT_T_YSJZSJB_ADJUST TA
...
NVL((SELECT FININTORG FROM CODE_T_AGENCY_SPF WHERE GUID = Y.AGENCYID),(SELECT DEPTID FROM SPF_V_FBASEINFO_A WHERE SPFID = Y.SPFID)) FININTORGGUID,
(SELECT
...
END
 FROM FASP_V_PUBEXPFUNC WHERE GUID = Y.EXPFUNCGUID) LOCEXPSTRGUID
    ...
    (
    SELECT * FROM (
   SELECT T1.*,NVL(EXPFUNCID,(SELECT EXPFUNCID FROM SPF_V_PBASEINFO_A WHERE PROJECTID = T1.PROJECTID)) EXPFUNCGUID  FROM }'||V_TABLE|| ' T1
   WHERE T1.STATUS = ''1'' AND T1.DBVERSION >= :1 AND PROJECTID <> ''*'' AND '||V_WHERE||' AND NOT EXISTS(SELECT 1 FROM BGT_T_YSLYWHB WHERE SUPERID = T1.DATAKEY)
  ...
  WHERE TA.SFJZ = '0' AND TA.ISWRITEOFF = '0' AND EXISTS(SELECT 1 FROM }'||Q'{(SELECT Y.FINYEAR,Y.AGENCYID AGENCYGUID,Y.PROJECTID PROGUID, EXPFUNCGUID,Y.EXPECOGUID,Y.FUNDSOURCEGUID,Y.AMT,Y.DATAKEY,Y.DBVERSION
   ...
   SELECT * FROM (
   SELECT T1.*,NVL(EXPFUNCID,(SELECT EXPFUNCID FROM SPF_V_PBASEINFO_A WHERE PROJECTID = T1.PROJECTID)) EXPFUNCGUID  FROM }'||V_TABLE|| ' T1
   WHERE T1.STATUS = ''1'' AND T1.DBVERSION >= :2 AND PROJECTID <> ''*'' AND '||V_WHERE||' AND NOT EXISTS(SELECT 1 FROM BGT_T_YSLYWHB WHERE SUPERID = T1.DATAKEY)
   ...
   BEGIN
    ...
    V_SQL := Q'{BEGIN
  FOR C_DATA IN (
  SELECT FINYEAR,PROGUID,AGENCYGUID,EXPECOGUID,FUNDSOURCEGUID,SFJZ,ISWRITEOFF,DATAKEY,EXPFUNCGUID,AMT FROM BGT_T_YSJZSJB_ADJUST TA WHERE TA.SFJZ = '1'  AND TA.ISWRITEOFF ='0'
  AND EXISTS (SELECT 1 FROM (SELECT Y.FINYEAR,Y.AGENCYID AGENCYGUID,Y.PROJECTID PROGUID,'01687gDiFwDjc' AGENCYEXPGUID, Y.EXPFUNCGUID EXPFUNCGUID,'01687E2hS1Edv' FUNDTYPEGUID,Y.EXPECOGUID,Y.FUNDSOURCEGUID,Y.AMT,Y.DATAKEY,Y.DBVERSION
     ...
     (
     SELECT *  FROM (
     SELECT T1.*,NVL(EXPFUNCID,(SELECT EXPFUNCID FROM SPF_V_PBASEINFO_A WHERE SPF_V_PBASEINFO_A.PROJECTID = T1.PROJECTID)) EXPFUNCGUID FROM }'||V_TABLE|| ' T1
     WHERE T1.STATUS = ''1'' AND T1.DBVERSION >= :1 AND T1.PROJECTID <> ''*'' AND '||V_WHERE||' AND NOT EXISTS(SELECT 1 FROM BGT_T_YSLYWHB WHERE BGT_T_YSLYWHB.SUPERID = T1.DATAKEY)
     ) WHERE (EXPECOID IS NOT NULL AND EXPFUNCGUID IS NOT NULL)
     ) TT
     UNPIVOT(AMT FOR FUNDSOURCEGUID IN('||V_TMP||'))) Y) TB
     WHERE (TA.DATAKEY = TB.DATAKEY AND TA.FUNDSOURCEGUID = TB.FUNDSOURCEGUID
     ...
     INSERT INTO BGT_T_YSJZSJB_ADJUST('||V_COLLIST||',AMT,SFJZ,ISWRITEOFF)
     SELECT '||V_COLLIST||Q'{,0-AMT AMT,'0' SFJZ,'1' ISWRITEOFF
     FROM BGT_T_YSJZSJB_ADJUST
     WHERE DATAKEY = C_DATA.DATAKEY AND FUNDSOURCEGUID = C_DATA.FUNDSOURCEGUID AND SFJZ = '1' AND ISWRITEOFF = '0';

     UPDATE BGT_T_YSJZSJB_ADJUST SET ISWRITEOFF = '1'
     WHERE DATAKEY = C_DATA.DATAKEY AND FUNDSOURCEGUID = C_DATA.FUNDSOURCEGUID AND SFJZ = '1' AND ISWRITEOFF = '0'
     ...
     INSERT INTO BGT_T_YSJZSJB_ADJUST(FINYEAR,AGENCYGUID,PROGUID,AGENCYEXPGUID,EXPFUNCGUID,FUNDTYPEGUID,EXPECOGUID,FUNDSOURCEGUID,AMT,DATAKEY,BGTTYPEGUID,SAFUNDSGUID,WH,FININTORGGUID,LOCEXPSTRGUID)
...
(SELECT DEPTID FROM SPF_V_FBASEINFO_A WHERE SPF_V_FBASEINFO_A.SPFID = Y.SPFID)) FININTORGGUID,
  (SELECT
CASE WHEN SUBSTR(CODE,1,3) >= '201' AND SUBSTR(CODE,1,3) <= '229' THEN (SELECT GUID FROM FASP_V_PUBLOCEXPSTR WHERE CODE = '01')
 WHEN SUBSTR(CODE,1,5) = '23001' THEN (SELECT GUID FROM FASP_V_PUBLOCEXPSTR WHERE CODE = '020101')
 WHEN SUBSTR(CODE,1,5) = '23002' AND SUBSTR(CODE,1,7) <> '2300251' THEN (SELECT GUID FROM FASP_V_PUBLOCEXPSTR WHERE CODE = '020102')
...
END
 FROM FASP_V_PUBEXPFUNC WHERE FASP_V_PUBEXPFUNC.GUID = Y.EXPFUNCGUID) LOCEXPSTRGUID
  ...
  SELECT *  FROM (
  SELECT T1.*,NVL(EXPFUNCID,(SELECT EXPFUNCID FROM SPF_V_PBASEINFO_A WHERE SPF_V_PBASEINFO_A.PROJECTID = T1.PROJECTID)) EXPFUNCGUID FROM }'||V_TABLE|| ' T1
  WHERE T1.STATUS = ''1'' AND T1.DATAKEY = C_DATA.DATAKEY AND T1.DBVERSION >= :2 AND T1.EXPECOID IS NOT NULL AND T1.PROJECTID <> ''*'' AND '||V_WHERE||'
...
END;';
    ...
    --不存在的数据插入
    V_SQL := Q'{INSERT INTO BGT_T_YSJZSJB_ADJUST(FINYEAR,AGENCYGUID,PROGUID,AGENCYEXPGUID,EXPFUNCGUID,FUNDTYPEGUID,EXPECOGUID,FUNDSOURCEGUID,AMT,DATAKEY,BGTTYPEGUID,SAFUNDSGUID,WH,FININTORGGUID,LOCEXPSTRGUID)
SELECT Y.FINYEAR,Y.AGENCYID AGENCYGUID,Y.PROJECTID PROGUID,'01687gDiFwDjc' AGENCYEXPGUID, Y.EXPFUNCGUID,(SELECT (SELECT FUNDTYPEID FROM BGT_T_FUNCTYPE2FUNDTYPE WHERE BGT_T_FUNCTYPE2FUNDTYPE.FUNCTYPEID = FASP_V_PUBEXPFUNC.FUNCTYPE) FROM FASP_V_PUBEXPFUNC WHERE FASP_V_PUBEXPFUNC.GUID = Y.EXPFUNCGUID) FUNDTYPEGUID,Y.EXPECOGUID,Y.FUNDSOURCEGUID,Y.AMT,Y.DATAKEY,'01687wARQ1YgH' BGTTYPEGUID
,(SELECT ZHUANH FROM BGT_T_YSLYWHB WHERE BGT_T_YSLYWHB.SUPERID = Y.DATAKEY AND BGT_T_YSLYWHB.YSLY = Y.FUNDSOURCEGUID AND ROWNUM < 2) SAFUNDSGUID
,(SELECT LYWH FROM BGT_T_YSLYWHB WHERE BGT_T_YSLYWHB.SUPERID = Y.DATAKEY AND BGT_T_YSLYWHB.YSLY = Y.FUNDSOURCEGUID AND ROWNUM < 2) WH
...
SELECT * FROM (
SELECT T1.*,NVL(EXPFUNCID,(SELECT EXPFUNCID FROM SPF_V_PBASEINFO_A WHERE SPF_V_PBASEINFO_A.PROJECTID = T1.PROJECTID)) EXPFUNCGUID FROM }'||V_TABLE|| ' T1
WHERE  T1.STATUS = ''1'' AND T1.DBVERSION >= :1 AND T1.PROJECTID <> ''*'' AND '||V_WHERE||'
) WHERE (EXPECOID IS NOT NULL AND EXPFUNCGUID IS NOT NULL) ) TT
 UNPIVOT(AMT FOR FUNDSOURCEGUID IN('||V_TMP||Q'{))) Y
 WHERE  AMT <> 0 AND NOT EXISTS(SELECT 1 FROM BGT_T_YSJZSJB_ADJUST
 WHERE BGT_T_YSJZSJB_ADJUST.DATAKEY = Y.DATAKEY AND BGT_T_YSJZSJB_ADJUST.FUNDSOURCEGUID = Y.FUNDSOURCEGUID)
 }';
    BEGIN
      EXECUTE IMMEDIATE V_SQL USING V_LASTMODIFIED;
    EXCEPTION WHEN OTHERS THEN
    ...
    AND T.OPERATION_TYPE = 'P_KEEP_ACCOUNTS9';

    INSERT INTO BGT_T_KEEPCOUNT_TIME(PROVINCE,YEAR,OPERATION_TYPE,LAST_OPERATION_TIME)
    VALUES(V_PROVINCE,V_YEAR,'P_KEEP_ACCOUNTS9',V_DBVERSION);
    RETURN;
  END P_KEEP_ACCOUNTS9;

--甘肃预算调整基本支出预算记账
--P_KEEP_ACCOUNTS9项目库对下转移记账
